...
## ROBERTA-BASE
# bash scripts/run.sh roberta-base classifier $dataset $big base $perf $server $num_samples $gpu_id
# ## DAPT
# bash scripts/run.sh $root_model_dir/$domain-roberta classifier $dataset $big dapt $perf $server $num_samples
# ## TAPT
bash scripts/run.sh $root_model_dir/$dataset/roberta-tapt/ classifier $dataset $big tapt $perf $server $num_samples $gpu_id
