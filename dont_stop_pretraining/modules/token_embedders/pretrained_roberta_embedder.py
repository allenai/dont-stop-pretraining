...
from supermask_bert.models.roberta.roberta_with_supermask import \
    RobertaModelWithSupermask
from supermask_bert.models.roberta.roberta_with_variable_mask import \
    RobertaModelWithVariableSupermask


@TokenEmbedder.register("pretrained_roberta")
class PretrainedTransformerEmbedder(TokenEmbedder):
    """
    Uses a pretrained model from ``pytorch-transformers`` as a ``TokenEmbedder``.
    """
    def __init__(self, supermask: bool, model_name: str = "roberta-base", max_length: int = None, supermask_type: str = None, requires_grad: bool = True) -> None:
        super().__init__()
        ...
        if supermask:
            self.transformer_model = RobertaModelWithVariableSupermask.from_pretrained("roberta-base")
        else:
            if 'supermask' in model_name.split("/")[-1]:
                self.transformer_model = RobertaModelWithVariableSupermask.from_pretrained("roberta-base")
                self.transformer_model.add_pretrained_supermask(os.path.join(model_name, 'supermask'), supermask_type)
            else:
                self.transformer_model = RobertaModel.from_pretrained(model_name)
    ...
    def add_supermask(self, type, sparsity):
        self.transformer_model.add_supermask(type, sparsity)

    def add_pretrained_supermask(self, path, type):
        self.transformer_model.add_pretrained_supermask(path, type)
